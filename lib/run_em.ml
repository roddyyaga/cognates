...
let run initialiser ?params_callback ~smoothing ~base_cognate_prob data_path
    name iterations () =
  ...
  in
  let rows = Dataset_utils.load_rows data_path in
  ...
  let expect =
    Em.expectations ~explain:false encoders decoders row_pairs weights_tables
      ~base_cognate_prob:(Probability.of_float base_cognate_prob)
  in
  ...
  let iterate n alpha theta =
    ...
    List.iter (List.range 0 1) ~f:(fun i ->
        let clusters =
          Scoring.cluster 0.01 (* + ((0.7 - 0.001) * of_int i / 20.0))*)
            score_graph
        in
        ...
        Owl.Dataframe.to_csv new_df (name ^ Printf.sprintf "_one_%d_%d.csv" i n));
    Stdio.print_endline "";
    (alpha, theta)
  in
  let _final_alpha, _final_theta =
    List.fold
      ...
      ~f:(fun (alpha, theta) n -> iterate n alpha theta)
      ~init:(alpha, theta)
  in
  ()

let print_sorted_thetas n _alpha theta =
  let sorted = Em.sorted_thetas theta in
  List.iter (Utils.take_last sorted n) ~f:(fun (taxons, phones, v) ->
      let t1, t2 =
      ...
      in
      Stdio.printf "%s %s %s %s %.4f\n" t1 t2 p1 p2 (Probability.Log.to_float v));
  Stdio.print_endline ""
