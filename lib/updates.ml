...
let find_pair hashtbl (x, y) =
  match Hashtbl.find hashtbl (x, y) with
  | Some v -> (v, false)
  | None -> (Hashtbl.find_exn hashtbl (y, x), true)
...
let matching_tokens encoders_table aligned_rows =
  ...
  List.iter aligned_rows
    ~f:(fun (first_row, second_row, (_scores, pointers, score)) ->
      ...
      List.iter [ List.hd_exn alignments ] ~f:(fun alignment ->
          ...
          in
          let counter =
            Hashtbl.find_or_add matches_table taxon_pair_key ~default:(fun () ->
                Hashtbl.create (module Tuple.Hashable_t (Int) (Int)))
          in
  ...
  let all_counts =
    List.map (Hashtbl.keys matches_table) ~f:(fun (first_taxon, second_taxon) ->
        let counter = matches_table.@![(first_taxon, second_taxon)] in
        List.map (Hashtbl.keys counter) ~f:(fun (x, y) ->
...
let print_frequent_matches encoders_table decoders_table weights_tables rows =
  ...
  |> List.iter ~f:(fun ((taxon1, taxon2), (phone1, phone2), score) ->
         ...
         if
           (String.(phone1_string <> "-" && phone2_string <> "-") || true)
           && String.(
                (taxon1 = "Russian" && taxon2 = "Italian")
                || (taxon1 = "Italian" && taxon2 = "Russian"))
         then
           Stdio.printf "%s %s %s %s %f\n" taxon1 taxon2 phone1_string
             phone2_string score)

let hashtbl_incr_float table key ~by =
  let current = Hashtbl.find_or_add table key ~default:(fun () -> 0.0) in
  let new_total = Float.(current + by) in
  Hashtbl.set table ~key ~data:new_total

let hashtbl_extend table key new_items =
  let current = Hashtbl.find_or_add table key ~default:(fun () -> []) in
  let new_value = current @ new_items in
  Hashtbl.set table ~key ~data:new_value

let all_taxon_phone_pair_scores encoders_table decoders_table phone_counts
    aligned_rows =
  let open Dataset_utils.Infix in
  let counter = Hashtbl.create (module Tuple.Hashable_t (String) (String)) in
  let () =
    matching_tokens encoders_table aligned_rows
    |> List.iter ~f:(fun ((taxon1, taxon2), (phone1, phone2), scores) ->
           ...
           let phones_key, _flipped =
             sort_tuple (phone1_string, phone2_string) ~compare:String.compare
           in
           hashtbl_extend counter phones_key scores)
  in
  let counter =
    Hashtbl.mapi counter ~f:(fun ~key ~data ->
        let t1, t2 = key in
        let get t =
...
let taxon_pair_total_scores encoders_table weights_tables rows =
  ...
  List.iter aligned_rows ~f:(fun (row1, row2, (_, _, score)) ->
      let (taxon1, taxon2), _flipped =
        sort_tuple (row1.taxon, row2.taxon) ~compare:String.compare
      in
      hashtbl_incr_float taxon_pair_totals (taxon1, taxon2) ~by:score);

  let open Dataset_utils.Infix in
  Hashtbl.keys taxon_pair_totals
...
let update_weights weights_table taxons phones update_function =
  ...
  Hashtbl.set weights_table ~key:taxons ~data:weights
